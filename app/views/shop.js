...
export async function renderShop(root) {
  ...
  function renderGrid() {
    const chronicles = state.chronicles || 0;
    gridEl.innerHTML = '';
    ...
    gridEl.querySelectorAll('.btn-buy').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          // Update state + topbar
          set({ chronicles: newChronicles });
          const ubChron = document.getElementById('ub-chronicles');
          if (ubChron) ubChron.textContent = newChronicles;
          showMsg(`✅ Pack acheté ! Il reste ✦ ${newChronicles}`, '#22c55e');
          renderGrid();
        } catch (err) {
          console.error(err);
