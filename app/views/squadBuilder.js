...
export async function renderSquadBuilder(root) {
  ...
  function chooseDifficultyThenFight() {
    ...
    overlay.querySelectorAll('.sb-diff').forEach(b => b.addEventListener('click', async () => {
      ...
      const m = await import('./squadBattle.js?v=13');
  ...
  topbar.querySelector('#sb-tuto').addEventListener('click', () => import('./squadTutorial.js?v=13').then(m => m.renderSquadTutorial(root)));
